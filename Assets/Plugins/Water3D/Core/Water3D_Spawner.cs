namespace Water3D {
	...
	public class Water3D_Spawner : MonoBehaviour
	{

		public static Water3D_Spawner instance;
		...
		/// </summary>
		public Vector3 initSpeed = new Vector3(1f,-1.8f);
        ...
        [ButtonAttribute("Start!", "Water3D.Water3D_Spawner", "RunSpawner")]public bool btn_0;
        ...
        [ButtonAttribute("Stop", "Water3D.Water3D_Spawner", "JustStopSpawner")] public bool btn_1;
        static void JustStopSpawner()
        {
            instance._breakLoop = true;

        }
        [ButtonAttribute("Stop and restore", "Water3D.Water3D_Spawner", "StopSpawner")] public bool btn_2;
        ...
        [ButtonAttribute("Help?", "Water3D.Water3D_Spawner", "askHelp")] public bool btn;
        static void askHelp()
        {
            string email = "info@2ddlpro.com";
            string subject = "Water 3D Help!";
		...
		void Start()
		{
			...
			WaterDropsObjects [0].GetComponent<MetaballParticleClass3D>().Active = false;
            ...
            for (int i = 1; i < WaterDropsObjects.Length; i++) {
				...
				WaterDropsObjects [i].GetComponent<MetaballParticleClass3D>().Active = false;
		...
		public void Restore()
		{
			...
			for (int i = 0; i < WaterDropsObjects.Length; i++) {
				if (WaterDropsObjects [i].GetComponent<MetaballParticleClass3D> ().Active == true) {
					WaterDropsObjects [i].GetComponent<MetaballParticleClass3D> ().Active = false;
				}
				WaterDropsObjects [i].GetComponent<MetaballParticleClass3D> ().witinTarget = false;			
			}
		...
		IEnumerator loop(Vector3 _pos, Vector3 _initSpeed, int count = -1, float delay = 0f, bool waitBetweenDropSpawn = true){
			...
			while (true) {
				for (int i = 0; i < WaterDropsObjects.Length; i++) {
					...
					MetaballParticleClass3D MetaBall = WaterDropsObjects [i].GetComponent<MetaballParticleClass3D> ();
